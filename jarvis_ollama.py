    ...
    **Gemini TTS** (if ``tts_backend`` is ``gemini`` and ``gemini_api_key`` is set)
    ...
    def _build_system_instruction(self) -> str:
        ...
        parts.append(
            ...
            "**Notepad / editors + writing:** To **launch** Notepad or Word, call **open_app** "
            "(``app_name``), **not** ``computer_control`` with ``action: \"open_app\"`` — the "
            "latter does not start programs. After the window is open, use **computer_control** "
            "``action: \"smart_type\"`` with ``text`` for the full poem/letter, **or** "
            "**file_controller** ``create_file`` / ``write``. **Never** tell the user to type "
            "it for you.\n"
